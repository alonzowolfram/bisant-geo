          ...
          for(cell in unique(immune_long$cell_type)) {
            ...
            if(all_pairwise) {
              ...
              emm <- tryCatch(
                ...
                error = function(e) {
                  warning(glue::glue("Error in estimating marginal means for model {formula} - {method} - {subset_var} - {subset_var_level} - {cell}: {e$message}"))
                  skip_to_next <<- TRUE
                }
              ...
              pairwise_results <- tryCatch(
                expr = emmeans::contrast(emm, method = "pairwise", adjust = "tukey") %>%
                  as.data.frame(),
                error = function(e) {
                  warning(glue::glue("Error in performing pairwise post-hoc comparisons for model {formula} - {method} - {subset_var} - {subset_var_level} - {cell}: {e$message}"))
                  skip_to_next <<- TRUE
                }
