...
if(!flagVariable(fgsea_graph_colors)) {
  ...
  if((length(fgsea_graph_colors) >= 2) & (all(areColors(fgsea_graph_colors[1:2])))) { # areColors() is found in helper_functions.R
    nes_palette <- colorRampPalette(c(fgsea_graph_colors[1], "white", fgsea_graph_colors[2]))(100)
  }
}
  ...
  for(subset_var_level in subset_var_levels) {
    ...
    for(model in models) {
      ...
      for(contrast in contrasts) {
        message(glue::glue("Subset variable {subset_var} | level {subset_var_level} | model {model} | contrast {contrast}"))
        # Subset. 
        ...
        df_sub$PathwayCleaned <- df_sub$pathway %>% 
          ...
          unlist() 
        # 2025/10/16: turning off sentence case for now, since it messes with acronyms like "DNA"
        # %>% 
        #   stringr::str_split(": ") %>% 
...
for(sv in names(plot_list_pathway_analysis)) {
  for(svl in names(plot_list_pathway_analysis[[sv]])) {
    for(model_num in names(plot_list_pathway_analysis[[sv]][[svl]])) {
      ...
      if(n > 16) {
        # Stop right there.
        warning(glue::glue("The combination of {sv} - {svl} - {model_num} has {n} plots, too many for graphing. Please graph these manually. Skipping to the next list of graphs"))
        rm(p_list)
        gc()
        next
      }
